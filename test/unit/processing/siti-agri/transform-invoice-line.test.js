...
describe('Transform invoice lines', () => {
  const testCases = [
    ...
    },
    {
      ...
      expected: { invoiceNumber: 'COHT0000001', value: 100, marketingYear: 2022, schemeCode: '80001', fundCode: 'DRD10', agreementNumber: 'AG0001', deliveryBody: 'RP00', description: 'G00 - Gross value of claim', accountCode: 'COHTacct' }
    }
  ]

  test.each(testCases)('transforms $scheme.schemeId invoice line', ({ lineData, scheme, expected }) => {
    const result = transformInvoiceLine(lineData, scheme)

    expect(result).toEqual(expected)
  })

  test('returns undefined values for empty lines', () => {
    for (const scheme of [LUMP_SUMS, COHT_CAPITAL]) {
      const result = transformInvoiceLine([], scheme)
