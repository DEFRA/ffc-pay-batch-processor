...
describe('Remove defunct participation fund', () => {
  ...
  const grossLine = (schemeCode, value) => ({
    description: GROSS_LINE_DESCRIPTION,
    schemeCode,
    value
  })

  const reductionLine = (schemeCode, value) => ({
  ...
  test.each([
    ['removes defunct fund if no other gross values', [
      grossLine(PARTICIPATION_PAYMENT_SCHEME_CODE, 5000),
      grossLine('80001', 0)
    ], 0, true],
    ['removes defunct fund if no other invoice lines', [
      grossLine(PARTICIPATION_PAYMENT_SCHEME_CODE, 5000)
    ], 0, true],
    ['does not remove fund if other gross values', [
      grossLine(PARTICIPATION_PAYMENT_SCHEME_CODE, 5000),
      grossLine('80001', 1000)
    ], 6000, false],
    ['removes fund if other net values all zero', [
      grossLine(PARTICIPATION_PAYMENT_SCHEME_CODE, 5000),
      grossLine('80001', 1000),
